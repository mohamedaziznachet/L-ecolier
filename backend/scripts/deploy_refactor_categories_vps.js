...
conn.on('ready', () => {
  console.log('✅ Connected via SSH! Refactoring categories on Hostinger VPS...');
  conn.sftp((err, sftp) => {
    if (err) throw err;

    const remoteScript = `${remotePath}/backend/refactor_categories_clean.js`;
...
function mapToMainCategory(catName, prodName = '') {
  ...
  // 6. Default: Fournitures scolaires (COUVERTURE PLASTIQUE, etc.)
...
async function run() {
  ...
  console.log(\`✅ Updated \${updatedCount} products into clean main categories!\`);
...
run().catch(console.error);
    ...
    writeStream.on('close', () => {
      ...
      const cmd = `
        ...
        node refactor_categories_clean.js
      `;
