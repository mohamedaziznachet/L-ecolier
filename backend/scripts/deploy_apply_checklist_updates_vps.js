...
conn.on('ready', () => {
  console.log('✅ Connected via SSH! Applying checklist brand logos & audit on Hostinger VPS...');
  conn.sftp((err, sftp) => {
    if (err) throw err;

    const remoteScript = `${remotePath}/backend/apply_checklist_updates.js`;
...
run().catch(console.error);
    ...
    writeStream.on('close', () => {
      ...
      const cmd = `
        ...
        node apply_checklist_updates.js
      `;
