...
conn.on('ready', () => {
  console.log('✅ Connected via SSH! Uploading frontend dist & publicRoutes.ts to Hostinger VPS...');
  conn.sftp((err, sftp) => {
    if (err) throw err;
    ...
    uploadDir(sftp, frontendDist, remoteDist, () => {
      ...
      const localPublic = 'c:/Users/DELL/Desktop/librerie l\'ecolier/backend/src/routes/publicRoutes.ts';
      const remotePublic = `${remotePath}/backend/src/routes/publicRoutes.ts`;

      console.log('Uploading publicRoutes.ts...');
      sftp.fastPut(localPublic, remotePublic, (err) => {
        if (err) throw err;
        console.log('✅ publicRoutes.ts uploaded!');
...
function restartVpsServer() {
  console.log('🚀 Rebuilding backend & restarting server on VPS...');
  const cmd = `
  ...
  conn.exec(cmd, (err, stream) => {
    ...
    stream.on('close', (code) => {
      console.log('Finished deploy with exit code:', code);
      console.log('Output:\n' + output);
      conn.end();
    }).on('data', (data) => {
      output += data;
