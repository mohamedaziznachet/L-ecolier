...
conn.on('ready', () => {
  console.log('✅ Connected via SSH! Deploying all updates to Hostinger VPS...');
  conn.sftp((err, sftp) => {
    if (err) throw err;
...
function uploadBackendFiles(sftp) {
  const files = [
    ...
    { local: 'c:/Users/DELL/Desktop/librerie l\'ecolier/backend/db_backup/pagesettings.json', remote: `${remotePath}/backend/db_backup/pagesettings.json` },
  ];
