...
export function AuthPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (mode === "login") {
        // ── User login (works for both admin and regular users) ────────────
        try {
          ...
          if ((response as any).require2FA) {
            setTwoFactorTempToken((response as any).tempToken || "");
          ...
          // If the logged in user is admin, set the admin flag and redirect to admin dashboard
          ...
          setErrorMsg(loginErr.message || "Adresse e-mail ou mot de passe incorrect.");
        }
        setIsLoading(false);
        return;
      }

      const emailTaken = await api.emailExists(formData.email);
      if (emailTaken) {
  ...
  const handle2FASubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!twoFactorCode.trim()) {
      setErrorMsg("Veuillez saisir le code de vérification.");
      return;
    }
    ...
    try {
      const response = await api.login(twoFactorEmail, formData.password, twoFactorCode, twoFactorTempToken);
      const user = response as any;
      ...
      setErrorMsg(loginErr.message || "Code de vérification invalide.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleForgotSubmit = async (event: React.FormEvent) => {
    ...
    if (forgotStep === 1) {
      ...
      try {
        ...
        setForgotStep(2);
        setSuccessMsg("E-mail envoyé. Veuillez saisir le code reçu et votre nouveau mot de passe.");
      } catch (err: any) {
    ...
    if (!forgotToken.trim()) {
      setErrorMsg("Veuillez saisir le code de réinitialisation (token).");
      return;
    }

    setIsLoading(true);
    try {
      await api.resetPassword(forgotEmail, forgotToken, newPassword);
            ...
            <div className="auth-form-wrap">
              ...
              <form onSubmit={handleForgotSubmit} className="auth-form">
                {forgotStep === 1 ? (
                  ...
                  <>
                    <Field htmlFor="forgotToken" label="Code de réinitialisation" required>
                      <div className="auth-input-wrap">
                        <KeyRound size={18} />
                        <input
                          id="forgotToken"
            ...
            <div className="auth-form-wrap">
              <div className="auth-heading">
                ...
                <div>
                  ...
                  <p>Un code de vérification a été envoyé à votre application d'authentification.</p>
                </div>
              </div>
              ...
              <form onSubmit={handle2FASubmit} className="auth-form">
                <Field htmlFor="twoFactorCode" label="Code de vérification" required>
                  <div className="auth-input-wrap">
                    <Lock size={18} />
                    <input
                      id="twoFactorCode"
                ...
                <button type="submit" className="auth-submit" disabled={isLoading}>
                  {isLoading ? <span className="auth-spinner" /> : <>Vérifier <ArrowRight size={16} /></>}
                </button>
